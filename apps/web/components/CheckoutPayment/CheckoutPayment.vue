  ...
  <fieldset class="md:mx-4 my-6" data-testid="checkout-payment">
    ...
    <div class="grid gap-4 grid-cols-2">
      <label v-for="{ id, icon, name, isSelectable } in paymentMethods.list" :key="id" class="relative">
        ...
        />
        <span
          class="h-20 flex flex-col items-center group justify-center py-4 px-4 cursor-pointer rounded-md border border-neutral-200 -outline-offset-2 hover:border-primary-200 hover:bg-primary-100 hover:text-primary-700 peer-focus:border-primary-200 peer-focus:bg-primary-100 peer-focus:text-primary-700 active:border-primary-300 active:bg-primary-200 peer-checked:outline peer-checked:outline-2 peer-checked:outline-primary-700 peer-disabled:opacity-50 peer-disabled:bg-neutral-100 peer-disabled:border-neutral-200 peer-disabled:cursor-not-allowed peer-disabled:[&_img]:grayscale"
        >
          <span v-if="id === -1">
          ...
          <p class="text-xs mt-2 text-secondary-500  group-hover:text-primary-700">{{ name }}</p>
        </span>
      </label>
    </div>
  </fieldset>
